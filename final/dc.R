...
dc <- read.csv("../data/dc_cleaned.csv") 
dc <- data.frame(dc[,-1]) %>% dplyr::select(-c("lat", "long", "bloom_date", "location", "alt", "MONTH", "ELEVATION", "LATITUDE", "LONGITUDE", "year", "DT00", "DP01", "DP10", "CDSD", "DSND", "DSNW", "TAVG", "DT32", "DX70", "EMSN", "EMXT", "HDSD", "DYFG", "EMNT", "PRCP"))



temp <- data.frame(t(c(80,rep(NA,10))))
names(temp) <- names(dc)
temp

dc <- rbind(dc, temp)

dc <- dc %>% get_forecasted_data()
annualWithPreds <- dc # For consistency
